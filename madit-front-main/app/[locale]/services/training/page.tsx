...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    fallbackTitle: categoryPage?.title || translate("training", locale as "sv" | "en"),
    fallbackDescription: categoryPage?.ingress || translate("training_description", locale as "sv" | "en"),
    locale,
...
export default async function TrainingPage({
  ...
  const [categoryPage, services] = await Promise.all([
    client.fetch<any>(SERVICE_CATEGORY_PAGE_GROQ(loc), { category: "training" }, { next: { revalidate: 60 } }),
    ...
    <>
      {categoryPage?.content && (
        <div className="container-width container-width-page small">
          <h1 className="heading-2">{categoryPage.title || translate("training", loc)}</h1>
